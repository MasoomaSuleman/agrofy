...
class EditAchievment extends StatefulWidget {
  final Achievements ach;
  final int index;
    EditAchievment({
    @required this.ach,
    @required this.index,
  ...
  @override
  _EditAchievmentState createState() => _EditAchievmentState();
}

class _EditAchievmentState extends State<EditAchievment> {
          ...
          child: Column(
            children: <Widget>[
               Container(
                child: Form(
                  key: _formkey,
                  child:AchievmentsPage(achievmentsItem: widget.ach,)),
              ),
              ClipRRect(
                ...
                child: FlatButton(
                  ...
                  onPressed: (){
                    ...
                    if (_expIsValid ) {
                      print(widget.ach.title);
                      setState(() {
                        _isLoading = true;
                      });
                      try{
                        currentUser
                            .editAchievment(widget.ach,context,currentUser,widget.index,oldach)
                            .then((value) {
                          if (value) {
  ...
  void _confirmDeleteAlertDialog(User currentUser) {
    ...
    onPressed: () {
      ...
      if (delete) {
        ...
        try{
          currentUser
              .deleteAchievment(context,currentUser,widget.index,oldach)
              .then((value) {
            if (value) {
              // new experince succefully added
            } else {
    ...
    AlertDialog alert = AlertDialog(
      title: Text("Are you sure you want to delete this achievement? "),
      actions: [
